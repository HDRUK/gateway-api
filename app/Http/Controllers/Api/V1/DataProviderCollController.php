    ...
    public function showSummary(Request $request, int $id): JsonResponse
    {
        try {
            ...
            $result = [
                'id' => $dpc->id,
                'name' => $dpc->name,
                'img_url' => (is_null($dpc->img_url) || strlen(trim($dpc->img_url) || (filter_var($dpc->img_url, FILTER_VALIDATE_URL)) === 0) ? $dpc->img_url : Config::get('services.media.base_url') . $dpc->img_url),
                'summary' => $dpc->summary,
                'enabled' => $dpc->enabled,
                'url' => $dpc->url,
                'service' => empty($service) ? null : $service,
