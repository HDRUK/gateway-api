...
class SocialLoginController extends Controller
{
    ...
    */
    ...
    {
        return $this->handleLogin($request, $provider, config('DTA_URL'), config('OPENATHENS_REDIRECT_DTA_URL'), true);
    }
    ...
    {
        return $this->handleLogin($request, $provider, config('gateway.gateway_url'), config('OPENATHENS_REDIRECT_URL'), false);
    ...
    private function handleLogin(Request $request, string $provider, string $baseRedirectUrl, $openAthensRedirectUrl, $isDTA): mixed
    {
        ...
        if (strtolower($provider) === 'openathens') {
            ...
            $oaUrl = config('OPENATHENS_ISSUER_URL') . '/oidc/auth?' . http_build_query($params);
            ...
            if ($isDTA) {
                ...
                if (config('APP_ENV') !== 'local') {
                    $providerURL = str_replace(config('app.url').'/api/v1/auth', config('DTA_API_URL').'/api/v1/auth/dta', $providerURL);
                }
    ...
    {
        $openAthensRedirectUrl = config('OPENATHENS_REDIRECT_URL');
        ...
        try {
            ...
            if (strtolower($provider) === 'openathens') {
                ...
                $oidc->providerConfigParam([
                    'authorization_endpoint' => config('OPENATHENS_ISSUER_URL') . '/oidc/auth',
                    'jwks_uri' => config('OPENATHENS_ISSUER_URL') . '/oidc/jwks',
                    'token_endpoint' => config('OPENATHENS_ISSUER_URL') . '/oidc/token',
                    'userinfo_endpoint' => config('OPENATHENS_ISSUER_URL') . '/oidc/userinfo',
                ]);
            ...
            if ($user['name'] === '' || $user['email'] === '') {
                return redirect()->away(config('DTA_URL') . '/account/profile')->withCookies($cookies);
            } else {
                $redirectUrl = session('redirectUrl');
                return redirect()->away($redirectUrl ?? config('DTA_URL'))->withCookies($cookies);
            }
        } catch (Exception $e) {
            Auditor::log([
