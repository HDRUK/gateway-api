            ...
            foreach ($collectionArray as $i => $collection) {
                ...
                foreach ($collectionModels as $model) {
                    if ((int)$collection['_id'] === $model['id']) {
                        ...
                        $collectionArray[$i]['image_link'] = (is_null($model['image_link']) || strlen(trim($model['image_link'])) === 0) ? null : (filter_var($model['image_link'], FILTER_VALIDATE_URL) ? $model['image_link'] : Config::get('services.media.base_url') . $model['image_link']);
                        $foundFlag = true;
                        break;
                    }
                }
