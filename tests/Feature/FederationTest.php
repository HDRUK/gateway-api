...
class FederationTest extends TestCase
{
    ...
    /**
     * enabled_at records the first time a federation is enabled and is
     * never cleared or overwritten by later enable/disable cycles.
     *
     * @return void
     */
    public function test_enabled_at_is_set_only_when_first_enabled(): void
    {
        $teamId = $this->createTeamForFederation();

        // A federation created disabled has no enabled_at
        $responseFederation = $this->json(
            'POST',
            self::TEST_URL_TEAM . '/' . $teamId . '/federations',
            [
        ...
        $responseFederation->assertStatus(201);
        $federationId = $responseFederation->decodeResponseJson()['data'];

        $this->assertNull(Federation::findOrFail($federationId)->enabled_at);
        ...
        // Enabling for the first time stamps enabled_at
        $firstEnabledAt = now()->addDay()->startOfSecond();
        $this->travelTo($firstEnabledAt);

        $this->json(
            'PUT',
            self::TEST_URL_TEAM . '/' . $teamId . '/federations/' . $federationId,
            array_merge($updatePayload, ['enabled' => true]),
            $this->header,
        )->assertStatus(200);

        $this->assertEquals(
            $firstEnabledAt->toDateTimeString(),
            Federation::findOrFail($federationId)->enabled_at->toDateTimeString()
        );

        // Disabling later leaves the original timestamp untouched
        $this->travelTo($firstEnabledAt->copy()->addDay());
        $this->json(
            'PUT',
            self::TEST_URL_TEAM . '/' . $teamId . '/federations/' . $federationId,
            array_merge($updatePayload, ['enabled' => false]),
            $this->header,
        )->assertStatus(200);
        ...
        // Re-enabling does not overwrite the original timestamp
        $this->travelTo($firstEnabledAt->copy()->addDays(2));
        $this->json(
            'PUT',
            self::TEST_URL_TEAM . '/' . $teamId . '/federations/' . $federationId,
            array_merge($updatePayload, ['enabled' => true]),
    ...
    /**
     * A federation created with enabled=true is stamped at creation time.
     *
     * @return void
     */
    public function test_enabled_at_is_set_when_created_enabled(): void
    {
        $teamId = $this->createTeamForFederation();
    ...
    private function createTeamForFederation(): int
    {
        $responseNotification = $this->json(
            'POST',
            self::TEST_URL_NOTIFICATION,
        ...
        $notificationID = $responseNotification->decodeResponseJson()['data'];
