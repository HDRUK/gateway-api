...
router.get("/", async (req, res) => {
	...
	if (req.query.form) {
		searchQuery = {
			$and: [
				{
					$or: [
						{ $and: [{ activeflag: "review" }, { authors: authorID }] },
						{ activeflag: "active" },
					],
				},
			],
	...
	await Promise.all([
		...
		var datasetCount =
			values[0].filter((d) => d.datasetfields.publisher !== "HDR UK").length ||
			0;
		...
		var datasetList = values[0].slice(
			datasetIndex,
			+datasetIndex + +maxResults
		);
		var toolList = values[1].slice(toolIndex, +toolIndex + +maxResults);
		var projectList = values[2].slice(
			projectIndex,
			+projectIndex + +maxResults
		);
	...
	if (type === "dataset") {
		...
		if (sample.length > 0) {
			...
			sample.split("::").forEach((filterTerm) => {
				filterTermArray.push({
					"datasetfields.physicalSampleAvailability": filterTerm,
				});
			});
			searchQuery["$and"].push({ $or: filterTermArray });
		}
		...
		if (geographicCoverage.length > 0) {
			...
			geographicCoverage.split("::").forEach((filterTerm) => {
				filterTermArray.push({
					"datasetfields.geographicCoverage": filterTerm,
				});
			});
			searchQuery["$and"].push({ $or: filterTermArray });
		}
...
function getFilterOptions(values) {
	...
	values[0].forEach((dataset) => {
		if (
			dataset.license &&
			dataset.license !== "" &&
			!licenseFilterOptions.includes(dataset.license)
		) {
			licenseFilterOptions.push(dataset.license);
		}

		if (
			dataset.datasetfields.physicalSampleAvailability &&
			dataset.datasetfields.physicalSampleAvailability.length > 0
		) {
		...
		if (
			dataset.datasetfields.publisher &&
			dataset.datasetfields.publisher !== "" &&
			!publisherFilterOptions.includes(dataset.datasetfields.publisher)
		) {
			publisherFilterOptions.push(dataset.datasetfields.publisher);
		}

		if (
			dataset.datasetfields.ageBand &&
			dataset.datasetfields.ageBand !== "" &&
			!ageBandFilterOptions.includes(dataset.datasetfields.ageBand)
		) {
			ageBandFilterOptions.push(dataset.datasetfields.ageBand);
		}

		if (
			dataset.datasetfields.geographicCoverage &&
			dataset.datasetfields.geographicCoverage !== "" &&
			!geographicCoverageFilterOptions.includes(
				dataset.datasetfields.geographicCoverage
			)
		) {
			geographicCoverageFilterOptions.push(
				dataset.datasetfields.geographicCoverage
			);
		}
	});
	...
	values[1].forEach((tool) => {
		if (
			...
			tool.categories.category !== "" &&
			!toolCategoriesFilterOptions.includes(tool.categories.category)
		) {
			toolCategoriesFilterOptions.push(tool.categories.category);
		}

		if (
			tool.categories.programmingLanguage &&
			tool.categories.programmingLanguage.length > 0
		) {
	...
	values[2].forEach((project) => {
		if (
			...
			project.categories.category !== "" &&
			!projectCategoriesFilterOptions.includes(project.categories.category)
		) {
