						...
						$and: [
							{ activeflag: 'active' },
							{
								'datasetfields.publisher': {
									$ne: 'OTHER > HEALTH DATA RESEARCH UK',
								},
							},
			...
			var q = RecordSearchData.aggregate([
				{
					$match: {
						$or: [
							{ 'returned.tool': { $gt: 0 } },
							{ 'returned.project': { $gt: 0 } },
							{ 'returned.person': { $gt: 0 } },
						],
					},
				},
				{
			...
			let resolvedArray = await Promise.all(
				topSearches.map(async (topSearch) => {
					...
					await Promise.all([
						...
						topSearch.datasets =
							resources[0][0] !== undefined &&
							resources[0][0].count !== undefined
								? resources[0][0].count
								: 0;
						topSearch.tools =
							resources[1][0] !== undefined &&
							resources[1][0].count !== undefined
								? resources[1][0].count
								: 0;
						topSearch.projects =
							resources[2][0] !== undefined &&
							resources[2][0].count !== undefined
								? resources[2][0].count
								: 0;
						topSearch.papers =
							resources[3][0] !== undefined &&
							resources[3][0].count !== undefined
								? resources[3][0].count
								: 0;
						topSearch.course =
							resources[4][0] !== undefined &&
							resources[4][0].count !== undefined
								? resources[4][0].count
								: 0;
					});
					return topSearch;
